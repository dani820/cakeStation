    ...
    <body>
		...
		<main>
			...
			<section id="content-box">
				<h2 id="sub-title">가입 신청 목록</h2>
		        <% if(list.isEmpty()) {%>
		        	<a id="list-null">회원가입 희망 매장이 없습니다.</a>
		        <% } %>
		        <% if(!list.isEmpty()) {%>
			        <table border="1" id="adminP-table" style="border-collapse:collapse">
				        ...
				        <% for(ShopMember sOne : list) { %>
					        <tr>
					        	...
					        	<td><a href="/admin/joinApproval?shop-id=<%= sOne.getShopId() %>" >[승인]</a></td>
					        	<td><a href="/admin/joinReject?shop-id=<%= sOne.getShopId() %>" >[거부]</a></td>
					        </tr>
				        <% } %>
